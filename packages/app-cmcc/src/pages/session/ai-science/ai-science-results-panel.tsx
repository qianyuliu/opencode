...
import { AI_SCIENCE_EXPERT_ID, AI_SCIENCE_LEAD_AGENT } from "./config"
import { AiScienceFilesTab, AiScienceTextReportTab, AiScienceVisualReportTab } from "./report-tabs"
import { AiScienceTeamTab } from "./team-tab"
import { useAiScienceWorkbench } from "./workbench-context"

type AiScienceTab = "team" | "files" | "text" | "visual"

const TABS: Array<{ id: AiScienceTab; label: string }> = [
  { id: "team", label: "分析团队" },
  { id: "files", label: "文件" },
  { id: "text", label: "文字报告" },
...
export function AiScienceResultsPanel(props: { onCreateSame?: () => void } = {}) {
  const context = useAiScienceWorkbench()
  const dockapi = useDockApi()
  const server = useServer()
  const serverSDK = useServerSDK()
  const tabs = useTabs()
  const [state, setState] = createStore({ active: "team" as AiScienceTab, manualReplayTab: false })
  ...
  const selectTab = (tab: AiScienceTab) => {
  ...
  const createSame = () => {
    ...
    tabs.newDraft({ server: server.key, directory, artifactDirectory, expertID: AI_SCIENCE_EXPERT_ID }, query, {
      agent: AI_SCIENCE_LEAD_AGENT,
    })
  ...
  return (
    <div class="@container relative flex size-full min-w-0 flex-col bg-[#f7f8fb]">
      <nav
        ...
        aria-label="AI for Science 结果页签"
      >
        <For each={TABS}>
          {(tab) => (
            <button
      ...
      <div class="min-h-0 flex-1 overflow-hidden" classList={{ "pb-[72px]": showReplayBar() }}>
        <Switch>
          ...
          <Match when={state.active === "text"}>
            <AiScienceTextReportTab />
          </Match>
          <Match when={state.active === "visual"}>
            <AiScienceVisualReportTab />
          </Match>
        </Switch>
      </div>
        ...
        <div class="pointer-events-none absolute inset-x-0 bottom-3 z-30 flex justify-center px-3">
          <div class="pointer-events-auto flex max-w-full min-w-0 items-center gap-2.5 rounded-[18px] border border-[#dfe5ef] bg-white/95 px-2.5 py-2 shadow-[0_8px_24px_rgba(29,45,78,0.14)] backdrop-blur">
            <div class="mr-0 flex shrink-0 items-center gap-2 whitespace-nowrap text-[13px] font-semibold text-[#2563eb] @min-[340px]:mr-2">
              ...
              <strong class="hidden font-semibold @min-[340px]:inline">科研分析</strong>
            </div>
...
function replayTab(stage: DeepTradingReplayStage): AiScienceTab {
  return stage === "files" ? "files" : "team"
}

function replayStageLabel(stage: DeepTradingReplayStage) {
  if (stage === "files") return "整理科研产物"
  return "科研分析"
}
