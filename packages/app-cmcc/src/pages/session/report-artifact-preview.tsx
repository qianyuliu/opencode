...
export function ReportArtifactPreview(props: {
  ...
  const reports = createMemo(() => {
    ...
    return [...items].sort((left, right) => Number(right.path === props.preferredPath) - Number(left.path === props.preferredPath))
  })
  const selected = createMemo(
    () => reports().find((artifact) => artifact.path === state.selectedPath) ?? reports()[0],
  )
  ...
  const previewUrl = createMemo(() => {
    const artifact = selected()
    if (!artifact || props.kind !== "visual") return
