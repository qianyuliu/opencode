...
import { SHOPPERS_EXPERT_ID, SHOPPERS_LEAD_AGENT } from "./config"
import { ShoppersFilesTab, ShoppersTextReportTab, ShoppersVisualReportTab } from "./report-tabs"
import { ShoppersTeamTab } from "./team-tab"
import { useShoppersWorkbench } from "./workbench-context"

type ShoppersTab = "team" | "files" | "text" | "visual"

const TABS: Array<{ id: ShoppersTab; label: string }> = [
  { id: "team", label: "分析团队" },
  { id: "files", label: "文件" },
  { id: "text", label: "文字报告" },
...
export function ShoppersResultsPanel(props: { onCreateSame?: () => void } = {}) {
  const context = useShoppersWorkbench()
  const dockapi = useDockApi()
  const server = useServer()
  const serverSDK = useServerSDK()
  const tabs = useTabs()
  const [state, setState] = createStore({ active: "team" as ShoppersTab, manualReplayTab: false })
  ...
  const selectTab = (tab: ShoppersTab) => {
  ...
  const createSame = () => {
    ...
    tabs.newDraft({ server: server.key, directory, artifactDirectory, expertID: SHOPPERS_EXPERT_ID }, query, {
      agent: SHOPPERS_LEAD_AGENT,
    })
  ...
  return (
    <div class="@container relative flex size-full min-w-0 flex-col bg-[#f7f8fb]">
      <nav class="flex h-12 shrink-0 items-end gap-1 border-b border-[#dfe3ea] bg-white px-3" aria-label="推荐报告页签">
        <For each={TABS}>
          {(tab) => (
            <button
              type="button"
      ...
      <div class="min-h-0 flex-1 overflow-hidden" classList={{ "pb-[72px]": showReplayBar() }}>
        <Switch>
          ...
          <Match when={state.active === "text"}>
            <ShoppersTextReportTab />
          </Match>
          <Match when={state.active === "visual"}>
            <ShoppersVisualReportTab />
          </Match>
        </Switch>
      </div>
        ...
        <div class="pointer-events-none absolute inset-x-0 bottom-3 z-30 flex justify-center px-3">
          <div class="pointer-events-auto flex max-w-full min-w-0 items-center gap-2.5 rounded-[18px] border border-[#dfe5ef] bg-white/95 px-2.5 py-2 shadow-[0_8px_24px_rgba(29,45,78,0.14)] backdrop-blur">
            <div class="mr-0 flex shrink-0 items-center gap-2 whitespace-nowrap text-[13px] font-semibold text-[#2563eb] @min-[340px]:mr-2">
              ...
              <strong class="hidden font-semibold @min-[340px]:inline">购买推荐</strong>
            </div>
...
function replayTab(stage: DeepTradingReplayStage): ShoppersTab {
