...
import { scopeZhengqiArtifacts, zhengqiPublicResearchTranscripts } from "./data"

const MESSAGE_PAGE_SIZE = 200
const MEMBER_IDS = new Set(ZHENGQI_MEMBERS.map((member) => member.id))

export type ZhengqiWorkbenchContextValue = {
...
const ZhengqiWorkbenchContext = createContext<ZhengqiWorkbenchContextValue>()

export function ZhengqiWorkbenchProvider(
  ...
  return <ZhengqiWorkbenchContext.Provider value={value}>{props.children}</ZhengqiWorkbenchContext.Provider>
}

export function useZhengqiWorkbench() {
...
function emptyWorkbench(loading: boolean, error?: string, rootSessionId = ""): AgentWorkbench {
  return {
    ...
    agents: ZHENGQI_MEMBERS.map((member) => ({ ...member, status: "waiting", markdown: "" })),
    nestedAgentSessions: [],
    nestedAgentSessionsLoading: false,
    stats: { elapsedMs: 0, uniqueSearchUrlCount: 0, expertCount: ZHENGQI_MEMBERS.length },
    artifacts: [],
    ambiguities: [],
    loading,
    error,
